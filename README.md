# Hi 👋, I'm Manikanth Banka

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?color=00F7FF&center=true&vCenter=true&lines=Cloud+%26+DevOps+Engineer;AI+%2F+ML+Enthusiast;AWS+%7C+Docker+%7C+CI%2FCD;Building+Scalable+Intelligent+Systems;Open+to+Work+🚀" />
</p>

---

## 🚀 About Me
💻 Cloud & DevOps Engineer with hands-on experience in AWS, Docker & CI/CD  
🤖 AI/ML enthusiast building intelligent, data-driven systems  
⚙️ Skilled in cloud-native, serverless, and ML-integrated applications  
📈 Focused on automation, scalability, and performance optimization  
🎯 Actively looking for Cloud / DevOps / AI Engineer roles  

---

## 🛠️ Tech Stack

### ☁️ Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![EC2](https://img.shields.io/badge/EC2-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white)
![Lambda](https://img.shields.io/badge/Lambda-FF9900?style=for-the-badge&logo=awslambda&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

### ⚙️ CI/CD & IaC
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)
![AWS CodePipeline](https://img.shields.io/badge/CodePipeline-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-623CE4?style=for-the-badge&logo=terraform&logoColor=white)

### 🤖 AI / ML
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SageMaker](https://img.shields.io/badge/AWS_SageMaker-FF9900?style=for-the-badge&logo=amazonaws&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine_Learning-102230?style=for-the-badge)

### 🐧 OS & Tools
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes&logoColor=white)

---

## 📌 Featured Projects

### 🔹 CI/CD Pipeline Automation
- Automated deployments using AWS CodePipeline, CodeBuild & CodeDeploy  
- Integrated monitoring with CloudWatch  

### 🔹 Cloud-Native MEAN Stack App
- Dockerized full-stack app with CI/CD  
- Deployed on AWS with Nginx & VPC  

### 🔹 Serverless Application
- Built using Lambda, API Gateway, DynamoDB  
- Optimized performance and reduced latency  

### 🔹 ML-Based Cloud Resource Prediction
- Built using AWS SageMaker (ARIMA, LSTM)  
- Enabled intelligent auto scaling & cost optimization  

---

## 📊 GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Manikanth1707&show_icons=true&theme=tokyonight" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Manikanth1707&theme=tokyonight" />
</p>

---

## 🏆 Achievements & Activity

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Manikanth1707&theme=algolia" />
</p>

---

## 👀 Profile Views

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Manikanth1707&label=Profile%20Views&color=blue&style=flat" />
</p>

---

## 📫 Connect With Me

- 🔗 LinkedIn: https://www.linkedin.com/in/manikanth-banka/  
- 📧 Email: manikanthbanka17@gmail.com  

---

## 🤝 Open to Work
🚀 Looking for Cloud / DevOps / AI Engineer roles  

---

⭐ From [Manikanth1707](https://github.com/Manikanth1707)
